...
TREEFILE_PATH="/etc/rpm-ostree-layer.yaml"

buildah rm "$WORKING_CONTAINER"
buildah from --name "$WORKING_CONTAINER" "$FCOS_IMAGE"
buildah copy "$WORKING_CONTAINER" "$TRANSPILED_IGN" "$TEST_IGN_PATH"
...
buildah run "$WORKING_CONTAINER" rpm-ostree compose container "$TREEFILE_PATH"
